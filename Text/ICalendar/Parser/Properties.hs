...
parseFreeBusy (ContentLine _ "FREEBUSY" o bs) = do
  typ <-
    maybe (return def) (parseFBType . CI.mk .: paramOnlyOne) $
      lookup "FBTYPE" o
...
parseTrigger (ContentLine _ "TRIGGER" o bs) = do
  ...
  case value of
    ...
    "DATE-TIME" -> do
      val <- mustBeUTC =<< parseDateTime Nothing bs
...
parseRelatedTo (ContentLine _ "RELATED-TO" o bs) = do
  ...
  typ <-
    maybe (return def) (parseRelationshipType . CI.mk .: paramOnlyOne) $
      lookup "RELTYPE" o
...
parseRequestStatus (ContentLine _ "REQUEST-STATUS" o bs) = do
  ...
  when (isNothing statcode)
    . throwError
    $ "parseRequestStatus: invalid code: " ++ show bs
  when (B.null rest)
    . throwError
    $ "parseRequestStatus: missing statdesc: " ++ show bs
  (statdesc, rest') <-
    (\(a, b) -> (,b) <$> valueOnlyOne a)
      <=< parseText'
      $ B.tail rest
  statext <-
    if B.null rest'
...
parseExDate (ContentLine _ "EXDATE" o bs) = do
  ...
  case typ of
    "DATE-TIME" -> do
      xs <- mapM (parseDateTime tzid) bs'
      return . ExDateTimes (S.fromList xs) $ toO o'
    "DATE" -> do
      xs <- mapM parseDate bs'
...
parseAttendee (ContentLine _ "ATTENDEE" o bs) = do
  ...
  attendeeCUType <-
    g (parseCUType . CI.mk .: paramOnlyOne) $
      lookup "CUTYPE" o
  attendeeMember <-
    g (S.fromList .: mapM (parseURI . T.unpack)) $
      lookup "MEMBER" o
  attendeeRole <- g (parseRole . CI.mk .: paramOnlyOne) $ lookup "ROLE" o
  attendeePartStat <-
    g (parsePartStat . CI.mk .: paramOnlyOne) $
      lookup "PARTSTAT" o
  attendeeRSVP <-
    maybe (return False) (parseBool . CI.mk <=< paramOnlyOne) $
      lookup "RSVP" o
  attendeeDelTo <-
    g (S.fromList .: mapM (parseURI . T.unpack)) $
  ...
  attendeeSentBy <-
    mapM (parseURI . T.unpack <=< paramOnlyOne) $
      lookup "SENT-BY" o
  ...
  attendeeDir <-
    mapM (parseURI . T.unpack <=< paramOnlyOne) $
      lookup "DIR" o
  attendeeLanguage <-
    mapM (Language . CI.mk .: paramOnlyOne) $
      lookup "LANGUAGE" o
  ...
  return Attendee {..}
  where
    g :: (Monad m, Default b) => (a -> m b) -> Maybe a -> m b
    g = maybe $ return def
...
parseAttachment (ContentLine _ "ATTACH" o bs) = do
  ...
  case val of
    Just "BINARY" -> do
      ...
      case enc of
        ...
        _ ->
          throwError $
            "parseAttachment: invalid encoding: "
              ++ show enc
    Nothing -> do
  ...
  where
    binF a@(x, _) = f a && x /= "VALUE" && x /= "ENCODING"
    f (x, _) = x /= "FMTTYPE"
...
parseRecurId dts (ContentLine p "RECURRENCE-ID" o bs) = do
  ...
  where
    err d r =
      throwError $
        "parseRecurId: DTSTART local time mismatch: "
          ++ show (d, r)
...
parseGeo (ContentLine _ "GEO" o bs) = do
  ...
  when (B.null long' || isNothing (lat >> long))
    . throwError
    $ "Invalid latitude/longitude: " ++ show bs
...
parseVersion (ContentLine _ "VERSION" o bs) = do
  ...
  when (isNothing maxver)
    . throwError
    $ "parseVersion: error parsing version: " ++ show maxver'
  if null minver''
    then return $ MaxICalVersion maxJ (toO o)
    else do
      when (isNothing minver)
        . throwError
