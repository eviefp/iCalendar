...
parseRange "THISANDPRIOR" = do
  tell ["THISANDPRIOIR RANGE is deprecated."]
  return ThisAndPrior
...
parseMime t =
  let m = mimeType .: parseMIMEType $ T.toStrict t
...
parseRecur dts =
  permute
    ...
    <* P.eof
  where
    freq = istring "FREQ=" *> frequency
    frequency =
      Secondly <$ istring "SECONDLY"
    ...
    until' = do
      txt <- manyTill P.anyChar (void (P.char ';') <|> P.eof)
      return . Just . Left $
        case dts of
          DTStartDateTime _ _ ->
