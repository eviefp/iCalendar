...
data Content
  = ContentLine P.SourcePos (CI Text) [(CI Text, [Text])] ByteString
...
parseText' bs = do
  ...
  case runParser ((,) <$> texts <*> getInput) () "text" bs of
    ...
    Right (x, r) ->
      return
        ( map (c . Bu.toLazyByteString) x,
          r
        )
  where
    texts = sepBy1 text (P.char ',') <|> return [mempty]
    text = do
      x <- P.satisfy isTSafe'
      case x of
        '\\' -> do
...
parseSimple k (ContentLine _ _ o bs) = do
  c <- valueOnlyOne =<< parseText bs
  return $ k c (toO o)
...
parseSimpleI k (ContentLine _ _ o bs) = do
  c <- asks dfBS2IText
  return $ k (c bs) (toO o)
...
-- specification, and 'OtherParams'.
parseAltRepLang ::
  (Text -> Maybe URI.URI -> Maybe Language -> OtherParams -> a) ->
  Content ->
  ContentParser a
...
parseSimpleDatesOrDateTimes dt d (ContentLine _ _ o bs) = do
  ...
  case typ of
    "DATE-TIME" -> do
      x <-
        S.fromList .: mapM (parseDateTime tzid) $
          B.split ',' bs
      return . dt x $ toO o'
    "DATE" -> do
      x <- S.fromList .: mapM parseDate $ B.split ',' bs
      return . d x $ toO o'
    _ -> throwError $ "Invalid type: " ++ show typ
...
otherProperties = do
  ...
  where
    lineToOtherProp (ContentLine _ n opts bs) =
...
neg =
  maybe id (\x -> if x == '-' then negate else id)
    <$> optional (P.oneOf "+-")
