...
printVCalendar VCalendar {..} = do
  ...
  ln $ do
    prop "VERSION" $ versionOther vcVersion -- Should be first for
    printValue vcVersion -- compatibility.
  ln $ do
    prop "PRODID" $ prodIdOther vcProdId
    text $ prodIdValue vcProdId
  ln $ do
    prop "CALSCALE" $ scaleOther vcScale
...
printTZProp name TZProp {..} = do
  ...
  ln $ do
    prop "TZOFFSETTO" $ utcOffsetOther tzpTZOffsetTo
    printUTCOffset $ utcOffsetValue tzpTZOffsetTo
  ln $ do
    prop "TZOFFSETFROM" $ utcOffsetOther tzpTZOffsetTo
...
printVAlarm va = do
  ...
  line "END:VALARM"
  where
    repAndDur = unless (vaRepeat va == def) $ do
...
instance IsProperty Completed where
  printProperty Completed {..} = ln $ do
    prop "COMPLETED" completedOther
    printValue completedValue

instance IsProperty DurationProp where
  printProperty DurationProp {..} = ln $ do
    prop "DURATION" durationOther
    printValue durationValue

instance IsProperty Repeat where
  printProperty Repeat {..} = ln $ do
    prop "REPEAT" repeatOther
...
instance IsProperty Description where
  printProperty Description {..} = ln $ do
    prop "DESCRIPTION" $
      toParam (AltRep <$> descriptionAltRep)
...
instance IsProperty Location where
  printProperty Location {..} = ln $ do
    prop "LOCATION" $
      toParam (AltRep <$> locationAltRep)
        <> toParam locationLanguage
        <> toParam locationOther
    text locationValue

instance IsProperty Organizer where
  printProperty Organizer {..} = ln $ do
    prop "ORGANIZER" $
      toParam (CN <$> organizerCN)
        <> toParam (Dir <$> organizerDir)
        <> toParam (SentBy <$> organizerSentBy)
        <> toParam organizerLanguage
        <> toParam organizerOther
...
instance IsProperty JournalStatus where
  printProperty s = ln $ do
    prop "STATUS" $ journalStatusOther s
    printValue s

instance IsProperty Summary where
  printProperty Summary {..} = ln $ do
    prop "SUMMARY" $
      toParam (AltRep <$> summaryAltRep)
        <> toParam summaryLanguage
        <> toParam summaryOther
    text summaryValue
...
instance IsProperty RRule where
  printProperty RRule {..} = ln $ do
    prop "RRULE" rRuleOther
...
instance IsProperty Resources where
  printProperty Resources {..} = ln $ do
    prop "RESOURCES" $
      toParam (AltRep <$> resourcesAltRep)
        <> toParam resourcesLanguage
        <> toParam resourcesOther
...
instance ToParam DTStart where
  toParam DTStartDateTime {..} =
    toParam dtStartDateTimeValue
...
instance ToParam RDate where
  toParam RDateDates {..} =
    [("VALUE", [(NoQuotes, "DATE")])]
      <> toParam rDateOther
  toParam RDatePeriods {..} =
    [("VALUE", [(NoQuotes, "PERIOD")])]
      <> toParam rDateOther
...
instance ToParam OtherParams where
  toParam (OtherParams l) = fromOP <$> S.toList l
    where
      fromOP (OtherParam x y) = (CI.original x, (Optional,) <$> y)
...
instance ToParam RecurrenceId where
  toParam RecurrenceIdDate {..} =
    [("VALUE", [(NoQuotes, "DATE")])]
      <> toParam recurrenceIdRange
      <> toParam recurrenceIdOther
  toParam RecurrenceIdDateTime {..} =
    toParam recurrenceIdDateTime
      <> toParam recurrenceIdRange
      <> toParam recurrenceIdOther
...
instance ToParam Attachment where
  toParam UriAttachment {..} =
    toParam attachFmtType
      <> toParam attachOther
  toParam BinaryAttachment {..} =
    toParam attachFmtType
      <> toParam attachOther
...
instance ToParam Trigger where
  toParam TriggerDuration {..} =
    toParam triggerOther
      <> toParam triggerRelated
  toParam TriggerDateTime {..} =
    toParam triggerOther
